# Mapa do vault (hub central)

Ponto de entrada humano e para o grafo do Obsidian. Conecte notas novas aqui para manter o grafo denso.

## Constituição e memória de trabalho

- [[CLAUDE-BRAIN]] — regras, templates, taxonomia
- [[_memory/progress]] — estado atual (agentes leem primeiro)
- [[00-index/estado-do-projeto]] — visão rápida do que é este vault
- [[docs/repo-inventory]] — árvore do repositório (gerado por script)

## Domínios (00–08)

- [[01-architecture/README]]
- [[02-backend/README]]
- [[03-frontend/README]]
- [[04-flows/README]]
- [[05-operations/README]]
- [[06-guidelines/README]]
- [[07-product/README]]
- [[08-team/README]]

## Documentação operacional

- [[docs/repo-inventory]]

## Grafo

Abra no Obsidian: **Visualização em gráfico** a partir desta nota ou do comando global de Graph.
